...
def test_e2e_sync_tools_skip_up_to_date(
    ...
    config.lock_file.update_tool_info(
        tool="mytool",
        platform="linux",
        arch="amd64",
        tag="v1.2.3",
    ...
    stored_info = config.lock_file.get_tool_info("mytool", "linux", "amd64")
...
def test_e2e_sync_tools_partial_skip_and_update(
    ...
    mytool_info = config.lock_file.get_tool_info("mytool", "linux", "amd64")
...
def test_e2e_sync_tools_force_re_download(tmp_path: Path, create_dummy_archive: Callable) -> None:
    ...
    config.lock_file.update_tool_info("mytool", "linux", "amd64", "1.2.3", "sha256")
    tool_info = config.lock_file.get_tool_info("mytool", "linux", "amd64")
...
def test_non_extract_single_binary_copy(
    ...
    tool_info = config.lock_file.get_tool_info("single-bin-tool", "linux", "amd64")
    assert tool_info is not None
...
def test_error_preparing_download(
    ...
    tool_info = config.lock_file.get_tool_info("error-tool", "linux", "amd64")
...
def test_download_file_request_exception(
    ...
    # Verify version store doesn't have an entry for this tool
    tool_info = config.lock_file.get_tool_info("download-error-tool", "linux", "amd64")
    assert tool_info is None
...
def test_tool_with_custom_tag(
    ...
    # Verify the version store was updated
    tool_info = config.lock_file.get_tool_info("tool", "linux", "amd64")
    assert tool_info is not None
    assert tool_info["tag"] == "v1.0.0"
