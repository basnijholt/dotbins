...
After running `dotbins sync`, shell integration scripts are created in `~/.dotbins/shell/` for various shells.
...
### Tool-Specific Shell Configuration

You can add tool-specific shell configuration code in your `dotbins.yaml` that will be automatically included in the shell scripts. These configurations will only be executed if the tool is available on your system.
...
This will add the configuration commands to the generated shell scripts, but they will only be executed if the corresponding tools are found in your PATH.
