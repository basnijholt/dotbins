...
_SAMPLE_CONFIG = f"""\
# This is a sample config file for dotbins.
...
def _initialize(config: Config, config_file: str | Path | None = None) -> None:
    ...
    if config_file is None:
        log(
            ...
            f" in the tools directory at `{tools_dir}`",
            "info",
            "🔧",
        )
        config_file = config.tools_dir / "dotbins.yaml"
        config_file.write_text(_SAMPLE_CONFIG)
        log("Contents of the config file:", "info", "📝")
        log(config_file.read_text(), "info")
...
def main() -> None:  # pragma: no cover
    ...
    try:
        ...
        if args.command == "init":
            _initialize(config, args.config_file)
